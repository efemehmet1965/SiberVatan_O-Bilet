...
protected $pages;              // array of page outlines
...
protected $iconv;              // flag indicating if iconv is available
...
protected function _dochecks()
{
	...
	if(extension_loaded('mbstring') && (ini_get('mbstring.func_overload') & 2))
		$this->Error('mbstring.func_overload is set to 2, 6 or 7 in php.ini');
	// Check libz
	if(!function_exists('gzcompress'))
		$this->Error('zlib extension is required');
}
...
protected function _putpage($n)
{
	...
	{
		$s = '/Annots [';
		$this->PageLinks[$n]['annots'] = $this->n + 1;
		foreach($this->PageLinks[$n] as $pl)
			$s .= $pl[5].' 0 R ';
		$s .= ']';
		$this->_put($s);
...
protected function _tounicodecmap($uv)
{
	...
	$s .= "begincmap\n";
	$s .= "/CIDSystemInfo\n";
	$s .= "<</Registry (Adobe) /Ordering (UCS) /Supplement 0>> def\n";
